...
ssize_t rtc_write (struct file * filp, const char *buf, size_t count, loff_t * f_pos)
{
...
ssize_t rtc_read (struct file * filp, char *buf, size_t count, loff_t * f_pos)
{
...
int my_init_clock (void)
{
  inuse = 0;
  major = register_chrdev (0, "myclock", &fops);
  printk ("Major number = %d \n", major);
  return 0;
}

void my_exit_clock (void)
{
  unregister_chrdev (major, "myclock");
}

module_init (my_init_clock);
module_exit (my_exit_clock);
