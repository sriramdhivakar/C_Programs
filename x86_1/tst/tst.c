...
static ssize_t write(struct file *filp, const char __user *buf, size_t ln, loff_t *pos)
{
	...
	printk(KERN_INFO"balance is %d\n",balance);
	printk(KERN_INFO"ln is %d\n",(int)ln);
	...
	if(bytes_to_do == 0)
	{
		printk(KERN_INFO"Reached end of the device\n");
		return -ENOSPC;
	}

	nbytes = bytes_to_do - copy_from_user(kbuf+(*pos),buf,ln);
	*pos += nbytes;
	return nbytes;
}
static ssize_t read(struct file *filp, char __user *buf, size_t ln, loff_t *pos)
{
	...
	nbytes = bytes_to_do - copy_to_user(buf,kbuf+(*pos),ln);
	*pos += nbytes;
	return nbytes;
}
